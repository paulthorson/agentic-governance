...
The quality bar for docs work to pass adversarial review.

## Required inputs

A piece of docs work must carry, or reference, each of these. A missing
input is a finding (UNVERIFIABLE for the checks it would support).

| Input | Purpose | Supports check |
|-------|---------|-----------------|
| `work` | The docs artifact under review | All checks |
...
1. **No irrecoverable harm.** No docs outcome that cannot be undone.
...
- factually wrong documentation, missing critical documentation, actionable documentation that misleads.
