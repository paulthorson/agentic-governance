...
### Changed
- **Get AG Adv CONCERN amend (Class A — still DRAFT, ):** Pay Adv CONCERNs on tip [#60](https://github.com/paulthorson/agentic-governance/pull/60) @ `86e98081` without absorbing unpaid counsel work: (1) party blank loud — Contracting party = `[UNPAID — licensed professional]` (Paul LOCK; no Paul-personal/LLC fill); (2) ungated GitHub Releases / `git clone` named loud as known enforceability weakness until acceptance gate gate; (3) §10 re-accept = returners only — not retro rewrite of possessed Apache-2.0 copies; (4) Prohibited uses — `[UNPAID — licensed professional]` empty placeholder; (5) DRAFT↔outline cross-notes; (6) README / docs README Terms SoT pointer `#57` @ `9b5bcd9` → `#60` @ `86e98081`. Paths: ``, ``. Production ToS still requires licensed lawyer.

### Added
- **Get AG (Class A — DRAFT, ):** Fuller LICENSE text for AG Counsel / Adv tear-through. Every section labeled **DRAFT — ; lawyer-review banner; Apache-2.0 public framework license; **no default anonymous telemetry** (opt-in only; **not required for download**); acceptance gate own free will / accept risk / as-is / Paul not liable; governing law **Delaware** only (no invented venue/city/forum); arbitration placeholders only (no DIY mechanics). Path: ``. Pointers from outline + `docs/README.md` + initiatives index. **Not** production ToS.
...
- **Get AG — Paul LOCKs amend (Class A — still DRAFT, ):** Amend ``: Apache-2.0 public framework license; **no default anonymous telemetry — opt-in only** (supersedes default-on basics for the download gate; telemetry not required for download); Install/Get AG acceptance gate = own free will, accept risk, as-is, Paul not liable; Delaware governing law retained; keep DRAFT — — lawyer review required banner. Link to fuller LICENSE file.
