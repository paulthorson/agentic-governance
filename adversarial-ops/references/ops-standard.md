# Ops & Reliability Standard

The quality bar for ops work to pass adversarial review.

## Required inputs

A piece of ops work must carry, or reference, each of these. A missing
input is a finding (UNVERIFIABLE for the checks it would support).

| Input | Purpose | Supports check |
|-------|---------|-----------------|
| `work` | The ops artifact under review | All checks |
...
1. **No irrecoverable harm.** No ops outcome that cannot be undone.
...
- irreversible deployment, no rollback path, no disaster recovery, unobservable in production.
