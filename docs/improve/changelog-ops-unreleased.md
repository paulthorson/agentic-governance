# Changelog ops archive — former `[Unreleased]` dump

Agent/ops face only. Moved off the visitor `CHANGELOG.md` face under
`ENTERPRISE_TW_CHANGELOG_HUMANIZE` (#184). Historical Keep-a-Changelog version
sections `0.1.1` / `0.1.0` remain in `CHANGELOG.md`.

Do not treat this file as the product changelog. Product notes live in
...
- **Cos memory template (operator LOCK Cos enhancement — now LIVE via [#64](https://github.com/paulthorson/agentic-governance/pull/64) @ `a9a4327`):** Baked-in operator
  ...
  **Separate from public AG product surface.** Adv may name SoT later. Keep private operator data out of AG git. Folded into the same Class A tip as Check 9 (no second PR).
...
- **Eng extract execute status (Class A companion):** After Eng plan [#56](https://github.com/paulthorson/agentic-governance/pull/56) **Cos ACCEPT MERGED LIVE** @ `e7bb36e` — extract execute **GO** (site-repo extract continues **in parallel**). Site face → `paulthorson/agentic-governance-site`; this repo remains feed/corpus publisher + framework download. Path: `docs/initiatives/marketing-site-extract-execute.md`. Settled Get AG posture: Apache-2.0 only; no acceptance gate.
...
- **SoT: `DESIGN_SYSTEM_FIRST` (Brand & Design Setup) — LIVE via `#45` / `ead012f`.** Cos
  ...
  **REJECTED**. **HOLD ACCEPT until Adv names the check** (`DESIGN_SYSTEM_FIRST`) then Cos
  ...
  Keep private operator data out of AG git. Out of scope: inventing a token feed; filling a concrete
...
- **Draft SoT: `DESIGN_AGENCY_BAR` — not live / not effective until Cos ACCEPT merge.** Cos
  ...
  **not** mark LIVE until Cos ACCEPT MERGED SHA. Superseded alias `SPECTACLE_NOT_CRAFT` —
  ...
  Keep private operator data out of AG git. Out of scope: dashboard pixel implementation; amending
...
- **Draft SoT: `RESEARCH_HCI` — not live / not effective until Cos ACCEPT merge.** Master's
  ...
  then Cos ACCEPT merge. **P0:** Keep private operator data out of AG git; no invented KPI
...
- **Draft SoT: `SELF_AUDIT_LOOP` — not live / not effective until Cos ACCEPT merge.** Cos 6pm
  ...
  **not** OpenClaw briefs. **Metrics (fail closed):** Cos/operator hand-recommended AG meta-gaps the
  ...
  `AUDIT_CLEAR`) = **0**. Keep private operator data out of AG git. Soft / deferred / tip / wiki-scar-only = REJECTED. SoT:
...
- **Draft SoT: five Adv-named locks — not live / not effective until Cos ACCEPT merge.**
  Soft / deferred / tip / wiki-scar-only = **REJECTED**. Keep private operator data out of AG git. **HOLD ACCEPT until Adv PASS.** Exactly these five Adv-named locks. No
...
- **Draft SoT: `VISUAL_STEP_STILLS` (Critic Check 8) — not live / not effective until Cos
  ...
  **Scope:** product UX surfaces only (marketing + app chrome); **all** product UX teams
  ...
  other product UX team) — **not** OpenClaw briefs. **P0:** Keep private operator data out of AG git
...
- **UX harness lock: Mermaid userflows + JTBD before Eng handoff (Cos ACCEPT B).**
  ...
  product UX epics only — **not** OpenClaw briefs. Keep private operator data out of AG git. Stop
...
- **Class A honesty flip — Check 9 / Check 10 / Cos memory LIVE after Cos ACCEPT [#64](https://github.com/paulthorson/agentic-governance/pull/64) @ `a9a4327`:** CoE Draft intake
  ...
  **Check 10 stays Cos checklist** — **NOT** fail-closed / stop-the-presses. Cos memory ASK
  (`private_git` OR `local_folder` at Cos seating) stays. No new checks invented. Keep private operator data out of AG git.
