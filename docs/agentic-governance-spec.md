...
| `DESIGN_SYSTEM_FIRST` (**Brand & Design Setup**) | **LIVE** via `#45` / `ead012f` | Cos LOCK Paul — Design, Experience, and Branding are paramount; Eng follows signed craft. Design system is FIRST Initiative deliverable. Research Scope (Q1–Q8) before comps hunt. Fresh diverse business-model-matched comps; cites state model-fit + diversity (not one peer); FAIL fixed AG comps (Pentagram/500/AXM) as all-teams default / copy-paste; cites internal-only. Stacks on `DESIGN_AGENCY_BAR` + `RESEARCH_HCI` + `RESEARCH_BEFORE_ENHANCE` + Check 7/8. Template: `adversarial-ux/assets/templates/design-system.md`. See harness + Research Critic lock. |
