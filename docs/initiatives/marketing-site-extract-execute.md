...
**Owner seat:** Eng (+ Ops for Vercel retire/redirect); Cos coordinates site PR; look pixels HOLD; Apache-2.0 LICENSE only (no acceptance gate) 
...
| Get AG download | Settled release posture: Apache-2.0 only; **no acceptance gate**. Get AG CTA pixels HOLD on #39 look only |
...
## operator LOCK 2026-09-14 — Get AG (settled)

Settled posture: free/open source Apache-2.0; **no acceptance gate**; LICENSE is the only use governor.
...
| **Eng HOLD (look)** | Get AG CTA **pixels** remain HOLD on #39 look craft. |
...
**Extract execute continues on the site repo in parallel** — AG companion docs / publisher posture here do **not** block site extract PRs.
...
4. Fold settled release posture (**no acceptance gate**; Apache-2.0 only; Get AG CTA pixels HOLD on #39 look) into execute status.
...
| 3a | Get AG download | **no acceptance gate**; Apache-2.0 only |
...
- Public release: Apache-2.0 LICENSE + NOTICE; no acceptance gate
