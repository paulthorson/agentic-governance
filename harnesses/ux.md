...
- **G2 addendum (Cos-locked prose on this tip — not `#118` lock land):** Existing HUD = sim / Installed capture then delta. New / missing screen = wireframe only if possible on G2 standards. Fake HUD = **FAIL**. Do **not** land `#118` as a separate lock here (Adv HOLD).
- **Stack:** Addition on `DESIGN_AGENCY_BAR` (**LIVE** `#43` / `7e9e0b6`) + `RESEARCH_HCI` (**LIVE** `#38` / `214ed5b`) + `DESIGN_SYSTEM_FIRST` (**LIVE** `#45` / `ead012f`) + improve LIVE `#87` @ `2ab4b17` + `#98` @ `fe27c4b` + tip stream `#100`–`#104` via PR [#105](https://github.com/paulthorson/agentic-governance/pull/105) MERGED @ `023abf9` — **not** a replacement / not a second CoE.
...
**Draft SoT until Cos ACCEPT of AG #113 — not live / not effective until ACCEPT.** Soft, deferred, tip-only, or wiki/scar-page-only language is **REJECTED**. Do not treat a narrative pass as acceptance (literal). Parent epic [#111](https://github.com/paulthorson/agentic-governance/issues/111).
...
- On operator-facing mocks / product UI (`UX_UI_CONSTITUTION` / `UX_LAWS_GATE`, draft until Cos ACCEPT of AG #111 + #113): if a mock is shown to the operator without UX-laws check against PRIMARY https://lawsofux.com + SECONDARY operator-cited keysjoao 30-law list + declared product design system + WCAG 2.x AA floor, or the operator secondary URL is dropped, or winging stills ship, or Existing HUD uses fake pixels instead of sim/Installed capture→delta, or new/missing screen invents HUD beyond G2-standards wireframe — **stop**; Cos primary **FAIL before operator GO**; Adv + critic **FAIL** (not accept) law-breaks / HCI breaks. Exact WCAG 2.1 vs 2.2 (`#115`). Do **not** land `#112`/`#114`/`#115`/`#116`/`#118` on this tip. Do not treat a narrative pass as acceptance. Cite LIVE `#43` @ `7e9e0b6` + LIVE `#38` @ `214ed5b` + LIVE `#45` @ `ead012f` + LIVE `#87` @ `2ab4b17` + LIVE `#98` @ `fe27c4b` + [#105](https://github.com/paulthorson/agentic-governance/pull/105) MERGED @ `023abf9`. Path: AMEND agency/HCI/DS_FIRST — not a second CoE.
