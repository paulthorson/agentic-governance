...
- **`COS_OPERATOR_PLAIN_ENGLISH`** (fail-closed, fleet; draft until Cos ACCEPT of AG #100): Cos→operator channel uses everyday words only — Cos owns ban-list judgment for “equivalent process slang”; Adv may challenge; operator never babysits the lexicon. Do not treat a narrative pass as acceptance. fleet-parity. Cite LIVE #87 @ `2ab4b17` + LIVE #98 @ `fe27c4b` + `COS_CRITICAL_THINKING`.
- **`HUB_UPLOAD_SEAT_PATH`** (fail-closed; draft until Cos ACCEPT of AG #101): Eng Uploads `.ehpk` to Hub then **STOP** (no Eng Install); Cos notifies operator; operator Installs via Even app only. Upload ≠ CLOSED; Install ≠ CLOSED. Do not treat a narrative pass as acceptance. Cite LIVE #87 @ `2ab4b17` + LIVE #98 @ `fe27c4b` + LIVE [#99](https://github.com/paulthorson/agentic-governance/pull/99) @ `4d73758` / `SHIP_WITHOUT_SENSOR` — do **not** cite unpaid `#100`.
...
- Use process slang in Cos→operator channel (chat / voice / digests / rare operator-expected GH comments) — **fail closed** (`COS_OPERATOR_PLAIN_ENGLISH`, draft until Cos ACCEPT of AG #100). Cos owns ban-list judgment; operator never babysits. Seat↔seat / Class A / improve-inbox / Adv / Eng tip SoT unbound. Do not treat a narrative pass as acceptance
- Ask the operator to Hub-login or Hub-upload on a routine ship, Install on the operator’s device as Eng, or imply that path in operator-facing text — **fail closed** (`HUB_UPLOAD_SEAT_PATH`, draft until Cos ACCEPT of AG #101). Eng Uploads then **STOP**; operator Installs via Even app only. Do not treat a narrative pass as acceptance
...
- **Scope:** **fleet** / G2 plugin seat (PM, UX, Eng, Cos). Not OpenClaw.
...
- **Scope:** **fleet** / G2 plugin seat (UX, QA, Cos). Not OpenClaw.
...
- **Scope:** Eng, Cos / G2 plugin seat + fleet. Not OpenClaw.
...
#### `COS_OPERATOR_PLAIN_ENGLISH` (fail-closed, fleet) — draft until Cos ACCEPT of AG #100

Cos→operator channel uses **everyday words only**. Fail-closed. Do not treat a narrative pass as acceptance. **Do not treat a narrative pass as acceptance** (literal). fleet-parity (Not: OpenClaw unless a later story Cos-locks fleet parity).
...
Does **not** bind: seat↔seat agent mail, Class A story/PR bodies, improve-inbox, Adv challenges, Eng tip SoT.

**Who stamps + detection:**
- **Primary:** Cos self-detects on send; if slang slipped, Cos files same-day improve story and restates in everyday words.
...
- **Not:** OpenClaw (unless later story Cos-locks fleet parity — ); seat↔seat / Class A / improve-inbox / Adv / Eng tip SoT; merging with `AI_SLOP_COPY_FAIL`; operator babysitting the lexicon.
- **Stack:** `COS_FEEDBACK_TO_IMPROVE` **LIVE** [#87](https://github.com/paulthorson/agentic-governance/pull/87) @ `2ab4b17` + `COS_IMPROVE_INBOX` **LIVE** [#98](https://github.com/paulthorson/agentic-governance/pull/98) @ `fe27c4b` + `COS_CRITICAL_THINKING` — addition, not replacement. Sibling to `COS_PROJECT_CONTEXT_REMINDER` (not collapsed).
- **Metric (fail closed):** count of Cos→operator messages that contain process slang in a calendar day. Target = **0**. Any count ≥1 = hold / same-day improve story. Do not treat a narrative pass as acceptance.
- **P0:** No secrets, keys, emails, PII, or absolute host paths in AG git. No personal names in tip SoT.
- **Cite:** [#100](https://github.com/paulthorson/agentic-governance/issues/100) Cos amend + [#87](https://github.com/paulthorson/agentic-governance/pull/87) LIVE @ `2ab4b17` + [#98](https://github.com/paulthorson/agentic-governance/pull/98) LIVE @ `fe27c4b` + `COS_CRITICAL_THINKING`. Draft until Cos ACCEPT of AG #100.

#### `HUB_UPLOAD_SEAT_PATH` (fail-closed) — draft until Cos ACCEPT of AG #101

Standing Hub ship path for glass / G2 Hub plugin seats. Fail-closed Class A framing. Do not treat a narrative pass as acceptance. **Do not treat a narrative pass as acceptance** (literal). Upload success alone never closes Look / visual prove.

**Standing path:**
1. Eng logs into Hub with credentials and **Uploads** the `.ehpk` (required).
2. Eng **STOP** — Eng does **not** Install on the operator’s device.
3. Cos notifies the operator that Private Upload is ready.
4. Operator Installs via the Even app only, then Looks on glasses when asked.

Stacks Hub Ship Upload-only / zombie-ops scar. Eng claiming “cannot Upload” = violation.

**Cite-fold LIVE sensors — Upload/Install ≠ CLOSED:** Cite-fold LIVE from pack [#99](https://github.com/paulthorson/agentic-governance/pull/99) @ `4d73758` / tip `3892b29`:
- `SHIP_WITHOUT_SENSOR`
- Look / `VISUAL_STEP_STILLS` / verbal keep ≠ optical close

**Upload ≠ CLOSED. Install ≠ CLOSED.** Stills + operator Look unpaid stays **fail closed** for optical/visual close. Do not soft-merge Install into optical close.

**Emergency / non-routine exception:** **Cos-locked: never for routine ships.** Operator Hub-upload is not a standing path. True Hub auth breakage: Eng escalates once to Cos for a one-time browser login help — still not “ask operator to upload the pack.” If Hub credentials are broken, Cos surfaces that as ops breakage; do not redefine the standing path.
...
- **Id:** `HUB_UPLOAD_SEAT_PATH`
- **Who stamps:** Eng (Upload required; STOP before Install). Cos (operator notify). Operator (Even-app Install + Look when asked). Adv may CONCERN.
- **Scope:** **all glass / G2 Hub plugin seats** (fleet G2 plugin apps that ship `.ehpk`) — wherever Eng mints Hub packs for glasses. Do not treat a narrative pass as acceptance.
- **Not:** Eng Install on operator device; operator Hub-login/upload as standing routine path; Upload or Install alone as CLOSED / Look close; soft-merge Install into optical close; unpaid `#100` cite; OpenClaw; ; auto-merge #26.
- **Stack:** `SHIP_WITHOUT_SENSOR` + Look / `VISUAL_STEP_STILLS` / verbal keep ≠ optical close — cite LIVE [#99](https://github.com/paulthorson/agentic-governance/pull/99) @ `4d73758`. Operator-facing Hub notifies use everyday words under Cos standing orders (even before `#100` lands). Cite LIVE #87 @ `2ab4b17` + LIVE #98 @ `fe27c4b` only for wording discipline — **do not cite unpaid `#100`**.
- **Metric (fail closed):** count of routine-ship events where a seat asks the operator to Hub-login or Hub-upload, OR Cos implies that path in operator-facing text. Target = **0**. Do not treat a narrative pass as acceptance.
- **P0:** No secrets, keys, emails, PII, or absolute host paths in AG git. No personal names in tip SoT.
...
**Cos lift + exceptions:** Cos lifts only with explicit Cos statement (operator-facing and/or Class A SoT) that this gate is lifted. **Out of scope (not exceptions that narrative-pass UI):** docs-only / non-UI Class A tips with no product UI pixels. **Emergency:** Cos-locked **never** narrative-pass UI Eng without mock+GO while this gate stands. Glass Look / Check 8 / craft bar (`#104`) remain separate unpaid/paid gates — lifting this story later does not auto-clear those.

**Mock alone ≠ Eng unlock (Cos-locked):** Operator mock/wireframe GO clears **mock-before-build** only. It does **not** by itself unlock Eng when Cos craft FAIL, glass Look unpaid, Check 8 unpaid, seat PARK, or `#104` craft bar unpaid still apply. Stacks `#104`; glass Look/Check 8 remain separate.
...
**Named priority remediation:** [redacted product] — path `public/tarot/` in product repo `new-desires-philly`. Explicit comps + agency-bar rematch + Cos mock GO before next [redacted product] UI Eng.
...
Fail-closed UX/UI constitution epic core. Do not treat a narrative pass as acceptance (literal). **Path:** **AMEND** LIVE `DESIGN_AGENCY_BAR` / `RESEARCH_HCI` / `DESIGN_SYSTEM_FIRST` — **not** a second CoE. Separate from #106 tip. Children `#112` / `#114` / `#115` / `#116` unpaid until Adv re-challenge PASS those. Do **not** fold `#107`–`#110` / `#117` / `#118` on this tip.
...
**G2 addendum (Cos-locked prose — not `#118` lock land):** Existing HUD = sim / Installed capture then delta. New / missing screen = wireframe only if possible on G2 standards. Fake HUD = **FAIL**.
...
- **P0:** No secrets, keys, emails, PII, or absolute host paths in AG git. No personal names in tip trailers.
- **Cite:** [#111](https://github.com/paulthorson/agentic-governance/issues/111) Cos amend + cites above.

#### `UX_LAWS_GATE` (fail-closed, fleet) — draft until Cos ACCEPT of AG #113

Every mock / wire must be built from and measured against Cos-locked UX-laws SoT + that product’s declared design system. Do not treat a narrative pass as acceptance (literal). Parent epic [#111](https://github.com/paulthorson/agentic-governance/issues/111).
...
- On Cos→operator channel (`COS_OPERATOR_PLAIN_ENGLISH`, draft until Cos ACCEPT of AG #100): if Cos uses process slang in operator-facing chat / voice / digests / rare operator-expected GH comments — **stop**; restates in everyday words and files same-day improve story. Cos owns ban-list judgment; Adv may challenge; operator never babysits. Seat↔seat / Class A / improve-inbox unbound. Exception: operator asks to explain a lock id — Cos may quote the id once + define in everyday words (not narrative-pass to keep jargon). Do not merge with `AI_SLOP_COPY_FAIL`. Metric: Cos→operator process-slang messages / day ≥1 = **fail closed**. Do not treat a narrative pass as acceptance. Cite [#87](https://github.com/paulthorson/agentic-governance/pull/87) LIVE @ `2ab4b17` + [#98](https://github.com/paulthorson/agentic-governance/pull/98) LIVE @ `fe27c4b` + `COS_CRITICAL_THINKING`.
- On routine Hub ship (`HUB_UPLOAD_SEAT_PATH`, draft until Cos ACCEPT of AG #101): if Eng Installs on the operator’s device, a seat asks the operator to Hub-login or Hub-upload, Cos implies that path in operator-facing text, or Upload/Install alone is treated as CLOSED / Look close — **stop**. Standing path: Eng Uploads `.ehpk` then **STOP**; Cos notifies; operator Installs via Even app only. Upload ≠ CLOSED; Install ≠ CLOSED. Cite LIVE [#99](https://github.com/paulthorson/agentic-governance/pull/99) @ `4d73758` / `SHIP_WITHOUT_SENSOR` + `VISUAL_STEP_STILLS`. Do **not** cite unpaid `#100`. Metric: routine-ship operator Hub-login/upload asks = **fail closed**. Do not treat a narrative pass as acceptance. Soft #92 CoE tidy. Cite [#87](https://github.com/paulthorson/agentic-governance/pull/87) LIVE @ `2ab4b17` + [#98](https://github.com/paulthorson/agentic-governance/pull/98) LIVE @ `fe27c4b`.
