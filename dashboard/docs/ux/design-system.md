# AG public website — design system
...
2. **Research hunt**
3. **Brand & Design Setup** (signed `design-system.md`) — #46 @ `cdf1c41` / #45 @ `ead012f`
4. **UX Canvas** — #48 @ `e9b4827` — **SEPARATE** gate after Brand & Design Setup (not an alias). Gothelf Lean UX Canvas v2 boxes 1–8 as-is ([`ux-canvas.md`](ux-canvas.md)). Before screens.

Then Check 7/8 stills / Eng.

Stacks DESIGN_SYSTEM_FIRST #45 @ `ead012f`. Do not invent alternate canvas boxes.
 
**DESIGN_SYSTEM_FIRST:** This packet is the website Initiative SoT before Eng pixels.

---

## Audience
1. **Operators first** — people running governed AI loops who need the board to stay honest. 
2. **Enterprise buyers second** — diligence readers who need calm proof, not hype.

## Promise
Visible loop. Ship only what clears. **Not** a sandbox, certification, or guarantee agents obey.

## Experience pillars
1. **Editorial calm** — deep void, type leads, ample air. Graph never crowds the sell line. 
2. **Alive instrument** — dense measured 3D knowledge graph; continuous slight drift; fluid. 
3. **Process clarity** — numbered chapters sell how the loop works. 
4. **DESIGN_AGENCY_BAR restraint** — hierarchy, type, space, micro-interaction. Spectacle FAIL.

## Brand — Sage instrument
- Graphite void + **one muted sage hue family** for the instrument. 
- **White wash inside** the active path stroke only (no beads, no marble, no competing mint/blue/orange). 
- Graph-as-art: spatial clusters, depth, perspective — not a flat kit or seat org-chart.

## Brand voice — Direct founder
- Short, confident imperatives. 
- Plain English. 
...
Public site / marketing chrome **must not** claim:
- Governance is **mechanical** or a **safety guarantee**
- CEO / harness **paces spend** as a hard dollar stop (only provider billing console is a hard $ limit; framework meters its own units)
- Adversaries as **enforcement** of the org (they **judge** / instruction + process)
- **Non-negotiable guarantees** (those are design goals; code gates are listed separately)
- Inflated counts / marketing outcomes not in the tree / [`docs/capability-report.md`](../../../docs/capability-report.md)

Public site **may** say, in Direct founder English:
- Visible loop; ship only what clears
- Instructions, review tools, and a few code gates
- Independent personal project
- Apache 2.0
- Operators own provider billing, network, secrets, human oversight

FAIL: enforcement-as-marketing · “hard spend cap” · “makes governance mechanical” · stills retip for this copy lock.

## Information design
- Fluid + faint white **IN** stroke (no beads) + **true 3D**. 
- Measured-only + anonymized codes (`Project 01…`, process words) = floor. 
- Hatch unpaid. Toast only on real ships. Idle counts stay.

## Type & tokens
| Role | Face | Notes |
| --- | --- | --- |
| Display | **Advercase** | Retro Garamond serif. Headlines, chapter titles, brand. Mock stills: EB Garamond stand-in until licensed webfont. |
| Body | **Geist** | Quiet sans. Lede, blurbs, panels, UI chrome. |
...
- `--wash: rgba(255,255,255,.5)` inside active stroke 
- Rain ≤10% atmosphere, gray codes only

## Layout hybrid — Process Instrument in the Void
- **A Editorial Void:** type-led quiet; air; Advercase. 
- **B Process Story:** numbered chapters 01–07 sell the loop. 
- **C Measured Instrument:** dense real fluid 3D KG beside chapters.

## Chapters (land)
01 Research — Find what matters before a brief exists. 
02 Brief — Turn findings into a job the loop can hold. 
03 Stills — Show the work before it ships. 
04 Challenge — Pressure the stills until they hold. 
05 Ship — Put an update on the board. 
06 Recap — Close the week with what landed. 
07 Next — Keep the loop open for unpaid work.
...
## FAIL closed
Beads / marble / confetti / glow-as-craft · colored hairball · flat SVG kit · diary land copy · scoreboard drill-downs · docs-only tip (`e38482b`) · geometric sans as Advercase · screens without this signed DS · public naming of research agencies/products (Pentagram / 500 / AXM / …) · **enforcement-as-marketing** / mechanical-governance / hard-$ spend claims (#62).

## Craft defense (sensor)
Munzner: one luminance channel for the week’s path. Hue scarcity (sage family only). 
