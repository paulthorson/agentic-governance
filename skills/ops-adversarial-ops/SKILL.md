---
name: ops-adversarial-ops
description: The worker skill for the adversarial-ops loop. Orchestrates a governed review: the ops-adversary agent reviews blind, the constitution gates, and only a human clears a veto.
---

# Adversarial Ops & Reliability — Worker Skill

This is the orchestration loop for reviewing ops work. It mirrors the
...
2. **Adversary review.** Spawn the `ops-adversary` agent. Give it the work, the
...
- `deployability-check`, `rollback-check`, `disaster-recovery-check`, `monitoring-check`, `oncall-readiness-check`.
