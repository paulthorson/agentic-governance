---
name: prod-adversarial-product
description: The worker skill for the adversarial-product loop. Orchestrates a governed review: the product-adversary agent reviews blind, the constitution gates, and only a human clears a veto.
---

# Adversarial Product & Market — Worker Skill

This is the orchestration loop for reviewing product work. It mirrors the
...
2. **Adversary review.** Spawn the `product-adversary` agent. Give it the work, the
...
- `market-assumption-check`, `user-need-check`, `competitive-position-check`, `business-case-check`, `market-risk-scan`.
