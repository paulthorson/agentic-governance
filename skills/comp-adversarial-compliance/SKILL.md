---
name: comp-adversarial-compliance
description: The worker skill for the adversarial-compliance loop. Orchestrates a governed review: the compliance-adversary agent reviews blind, the constitution gates, and only a human clears a veto.
---

# Adversarial Compliance — Worker Skill

This is the orchestration loop for reviewing compliance work. It mirrors the
...
2. **Adversary review.** Spawn the `compliance-adversary` agent. Give it the work, the
...
- `regulatory-gate-check`, `policy-alignment-check`, `compliance-evidence`, `audit-trail-check`, `liability-scan`.
