# Product & Market Standard

The quality bar for product work to pass adversarial review.

## Required inputs

A piece of product work must carry, or reference, each of these. A missing
input is a finding (UNVERIFIABLE for the checks it would support).

| Input | Purpose | Supports check |
|-------|---------|-----------------|
| `work` | The product artifact under review | All checks |
...
1. **No irrecoverable harm.** No product outcome that cannot be undone.
...
- unvalidated market assumption, fabricated user need, unsupported competitive claim, unsound business case.
